...
int ssh_connect(ssh_session session)
{
    ...
    if (session->opts.fd != SSH_INVALID_SOCKET) {
        ...
        ret = SSH_OK;
    } else if (session->opts.ProxyCommand != NULL && strncmp(VBOX_PROXY_PREFIX, session->opts.ProxyCommand, VBOX_PROXY_PREFIX_LENGTH) == 0) {
        ret = ssh_socket_connect_proxycommand_vbox(session->socket,
                session->opts.host,
                session->opts.port > 0 ? session->opts.port : 22,
                session->opts.ProxyCommand + VBOX_PROXY_PREFIX_LENGTH);
#ifndef _WIN32
#ifdef HAVE_PTHREAD
