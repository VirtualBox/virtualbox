...
/**
 * @internal
 * @brief Open a socket with VirtualBox-specific ProxyCommand.
 * This call will always be nonblocking.
 * @param s    socket to connect.
 * @param command Command to execute.
 * @returns SSH_OK socket is being connected.
...
int
ssh_socket_connect_proxycommand_vbox(ssh_socket s, const char *host,
                                     uint16_t port, const char *params)
{
    socket_t fd;
    SOCKADDR_IN sa_proxy;
    int rc;
    char buffer[2048];
    ...
    /* Use buffer to convert parameters */
    strncpy(buffer, params, sizeof(buffer));
    char *proxy_addr = strchr(buffer, ' ');
    if (!proxy_addr)
    {
        SSH_LOG(SSH_LOG_WARNING, "Invalid proxy parameter string '%s'", params);
        return SSH_ERROR;
    }
    else
        *proxy_addr++ = 0; /* Terminate proxy type */

    if (strcmp(buffer, "HTTP"))
    {
        SSH_LOG(SSH_LOG_WARNING, "Unsupported proxy type '%s'", buffer);
        return SSH_ERROR;
    }

    /* Continue to parse parameters */
    char *proxy_auth = NULL;
    char *proxy_user = NULL;
    char *proxy_password = NULL;
    char *pszPort = strchr(proxy_addr, ' ');
    if (pszPort)
    {
        *pszPort++ = 0; /* Terminate IP address */

        proxy_user = strchr(pszPort, ' ');
        if (proxy_user)
        {
            *proxy_user++ = 0; /* Terminate port */
            proxy_password = strchr(proxy_user, ' ');
            if (proxy_password)
            {
                *proxy_password++ = ':'; /* Create user:password string */
                proxy_auth = bin_to_base64(proxy_user, strlen(proxy_user));
            }
        }

        sa_proxy.sin_port = htons(atoi(pszPort));
    }
    else
        sa_proxy.sin_port = htons(80);  /* Is this a reasonable default? */
    sa_proxy.sin_family = AF_INET;
    if (!inet_pton(sa_proxy.sin_family, proxy_addr, &sa_proxy.sin_addr))
    {
        SSH_LOG(SSH_LOG_WARNING, "Failed to convert '%s' to IPv4 address", proxy_addr);
        if (proxy_auth)
            free(proxy_auth);
        return SSH_ERROR;
    }
    ...
    proxy_addr = pszPort = proxy_user = proxy_password = NULL;
    /* Use buffer to compose HTTP CONNECT request */
    if (proxy_auth)
    {
        sprintf(buffer,
    ...
    SSH_LOG(SSH_LOG_PROTOCOL, "Connecting to '%s:%u' via proxy at '%s'", host, port, params);
    fd = socket(sa_proxy.sin_family, SOCK_STREAM, IPPROTO_TCP);
    if (fd < 0) {
        return SSH_ERROR;
    }

    rc = connect(fd, (SOCKADDR*)&sa_proxy, sizeof(sa_proxy));
    if (rc != 0)
    {
        SSH_LOG(SSH_LOG_WARNING, "Failed to connect to '%s:%u' via proxy at '%s'", host, port, params);
        closesocket(fd);
        return SSH_ERROR;
    }

    /* Send HTTP CONNECT */
    ssize_t cb = send(fd, buffer, strlen(buffer), 0);
    if (cb < 0)
    {
        SSH_LOG(SSH_LOG_WARNING, "Failed to send connect request to proxy at '%s'", params);
        closesocket(fd);
        return SSH_ERROR;
    }

    /* Recieve reply */
    cb = recv(fd, buffer, sizeof(buffer) - 1, 0); /* Limit to have enough room for ensuring zero termination of the response. */
    if (cb <= 0)
    {
        SSH_LOG(SSH_LOG_WARNING, "Failed to receive connect response from proxy at '%s'", params);
        closesocket(fd);
        return SSH_ERROR;
    }

    buffer[cb] = 0; /* Make sure it is zero-terminated */
    SSH_LOG(SSH_LOG_PACKET, "Received response from proxy:\n%s", buffer);

    /* Is this really necessary? */
    rc = ssh_socket_set_nonblocking(fd);
    if (rc < 0) {
        ssh_set_error(s->session, SSH_FATAL,
